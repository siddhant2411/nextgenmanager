...
public interface InventoryInstanceRepository extends JpaRepository<InventoryInstance,Long> {
    ...
    @Query(value = "SELECT * FROM inventoryInstance i WHERE i.inventoryItemRef = :inventoryItemId AND i.quantity>=:qty ORDER BY i.entryDate ASC LIMIT 1", nativeQuery = true)
    public InventoryInstance findLatestInventoryInstance(@Param("inventoryItemId") int inventoryItemId,@Param("qty") double qty);



    @Query(value = "SELECT COUNT(*) FROM inventoryInstance i WHERE i.inventoryItemRef = :inventoryItemId AND i.quantity>0",nativeQuery = true)
